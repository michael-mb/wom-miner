...
def find_lehrshuhl():
    ...
    docs_with_lehr = es.search(index=index_name_uni, query=query, size=500, sort=[{"name.keyword": "asc"}])
    ...
    while (current_i_max >= 499):
        ...
        docs_with_lehr = es.search(index=index_name_uni, query=query, size=500, search_after=current_last_sort, sort=[{"name.keyword": "asc"}])
    ...
    for id in ids:
        ...
        if current_term.get('term_vectors').get('html').get('terms').get('lehrstuhl') != None:
            lehr_deu = current_term.get('term_vectors').get('html').get('terms').get('lehrstuhl').get('tokens')
        if current_term.get('term_vectors').get('html').get('terms').get('chair') != None:
            lehr_eng = current_term.get('term_vectors').get('html').get('terms').get('chair').get('tokens')
        if current_term.get('term_vectors').get('html').get('terms').get('abteilung') != None:
            abt_deu = current_term.get('term_vectors').get('html').get('terms').get('abteilung').get('tokens')
        if current_term.get('term_vectors').get('html').get('terms').get('department') != None:
            abt_eng = current_term.get('term_vectors').get('html').get('terms').get('department').get('tokens')
    ...
    docs_with_multiple_lehr = es.search(index=index_name_uni, query=q, size=500, sort=[{"name.keyword": "asc"}])
    ...
    for i in range((current_multiple_i_max + 1)):
        current_html = docs_with_multiple_lehr.get('hits').get('hits')[i].get('_source').get('html')
    ...
    while (current_multiple_i_max >= 499):
        ...
        # get next result-page
        docs_with_multiple_lehr = es.search(index=index_name_uni, query=q, size=500, search_after=current_multiple_last_sort, sort=[{"name.keyword": "asc"}])
