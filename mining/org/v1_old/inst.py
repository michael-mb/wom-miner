...
# local
es = Elasticsearch({'host': 'localhost', 'port': 9200, 'scheme': 'https'}, basic_auth=('elastic', 'elastic'),
                   verify_certs=False, ssl_show_warn=False)
# uni-server
# es = Elasticsearch({'host':'beta.mowesta.com', 'port': 443, 'path_prefix':'elastic', 'scheme':'https'},  basic_auth=('elastic', 'nctLD+JV+HGl+YDEmAoc'))
...
def find_institutes():
    ...
    docs_with_inst = es.search(index=index_name_uni, query=query, size=500, sort=[{"name.keyword": "asc"}])
    ...
    while (current_i_max >= 499):
        ...
        docs_with_inst = es.search(index=index_name_uni, query=query, size=500, search_after=current_last_sort, sort=[{"name.keyword": "asc"}])
    ...
    for id in ids:
        ...
        html_string = current_doc.get('_source').get('html')
        ...
        if current_term.get('term_vectors').get('html').get('terms').get('Institut') != None:
            inst_deu = current_term.get('term_vectors').get('html').get('terms').get('Institut').get('tokens')
        if current_term.get('term_vectors').get('html').get('terms').get('institute') != None:
            inst_eng = current_term.get('term_vectors').get('html').get('terms').get('institute').get('tokens')
        if current_term.get('term_vectors').get('html').get('terms').get('zentrum') != None:
            zentrum_deu = current_term.get('term_vectors').get('html').get('terms').get('zentrum').get('tokens')
        if current_term.get('term_vectors').get('html').get('terms').get('center') != None:
            zentrum_eng = current_term.get('term_vectors').get('html').get('terms').get('center').get('tokens')
        ...
        for i in range(len(inst)):
            ...
            sort_out_inst_words = ["Institut", "Institute", "institute", "center", "Center", "Zentrum", "Zentrum für", "center for", "Center for", "Institut für", "Institute für", "institute for", "Institute for", "Institute for Computer"]
    ...
    for i in range(len(instis_sorted)):
        ...
        if len(result_link_search.get('aggregations').get('duplicateWords').get('buckets')) > 0:
            instis_Link.append(result_link_search.get('aggregations').get('duplicateWords').get('buckets')[0].get('key'))
        else:
    ...
    sh = []
    for a in instis:
        #sh.append({"query_string": {"query": a.get('key'), "default_field": "html"}})
        sh.append({"match_phrase": {"html": a}})

    q = {}

    q["bool"] = {"should": sh, "minimum_should_match": 2} # todo -> minimum_should_match -> evaluate a good value

    docs_with_multiple_inst = es.search(index=index_name_uni, query=q, size=500, sort=[{"name.keyword": "asc"}])
    ...
    for i in range((current_multiple_i_max + 1)):
        current_html = docs_with_multiple_inst.get('hits').get('hits')[i].get('_source').get('html')
                            ...
                            if outer_struct.find("]", last_o) < outer_struct.find("[", last_o) or (
                                ...
                                if inner_struct.find("]", last_o) < inner_struct.find("[", last_o) or (
                                    ...
                                    if outer_struct.rfind("[", 0, last_o) > outer_struct.rfind("]", 0, last_o) or (
                                            outer_struct.rfind("[", 0, last_o) != -1 and outer_struct.rfind("]", 0, last_o) == -1):
                                        if inner_struct.rfind("[", 0, last_o) > inner_struct.rfind("]", 0, last_o) or (
                                                inner_struct.rfind("[", 0, last_o) != -1 and inner_struct.rfind("]", 0, last_o) == -1):
            ...
            while True:
                ...
                if found == []:
                    ...
                    if extra_links[-1] == None:
                        ...
                        extra_links[-1] = docs_with_multiple_inst.get('hits').get('hits')[i].get('_source').get('name') + extra_links[-1]  # link relativ -> add prefix
    ...
    while (current_multiple_i_max >= 499):
        ...
        # get next result-page
        docs_with_multiple_inst = es.search(index=index_name_uni, query=q, size=500, search_after=current_multiple_last_sort, sort=[{"name.keyword": "asc"}])
